...
function CategoryTree({ categories, selectedId, onSelect }) {
  ...
  // Auto-expand to selected category
  useEffect(() => {
...
/* ── Image upload card ────────────────────────────────────────────────────── */
function ImageUpload({ label: lbl, required, multiple = false, name, onChange, icon: IconComp, color = "indigo" }) {
...
export default function NewProductPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const { data } = await api.get("/api/products/next-styleid");
        if (!cancelled) setForm((f) => ({ ...f, productStyleID: data.productStyleId }));
      } catch (e) {
        /* silent */
      }
    })();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { data: created } = await api.post("/api/products", body);
        ...
        <div className="grid grid-cols-12 gap-0 min-h-[calc(100vh-64px)]">
          ...
          <div className="col-span-5 bg-white p-6 overflow-y-auto space-y-4 border-l border-slate-100">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <ImageUpload label="Front Image" required name="front" onChange={handleImageChange} icon={Package} color="purple" />
                <ImageUpload label="Back Image" required name="back" onChange={handleImageChange} icon={Package} color="blue" />
                <ImageUpload label="Left Side" required name="leftSide" onChange={handleImageChange} icon={Package} color="cyan" />
                <ImageUpload label="Right Side" required name="rightSide" onChange={handleImageChange} icon={Package} color="emerald" />
                <ImageUpload label="Zoomed Image" required name="zoomed" onChange={handleImageChange} icon={Image} color="amber" />
              </div>
            </div>
