...
const FIELDS = [
  ...
  { key: "customerAddress", label: "Address", wide: true },
];

const COLUMNS = ["customerName", "customerCode", "contactPerson", "customerEmail", "customerPhone"];
...
export default function CustomersPage() {
  ...
  const saveItem = async () => {
    if (!String(formData.customerName ?? "").trim()) return;
    setSaving(true);
    try {
      const body = {};
      FIELDS.forEach((f) => {
      ...
      if (editingItem) {
        await api.put(`/api/customers/${editingItem.id}`, body);
      } else {
  ...
  const deleteItem = async (item) => {
    if (!window.confirm(`Delete customer "${item.customerName}"?`)) return;
    try {
        ...
        <Card className="border-slate-200 bg-white shadow-sm">
          <CardHeader className="flex flex-row items-center justify-between space-y-0">
            <div className="flex items-center gap-2">
              ...
              <CardTitle className="text-base text-slate-900">Customers</CardTitle>
