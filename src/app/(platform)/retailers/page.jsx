...
import {
  ...
  Plus, Search, ShoppingBag, Trash2, X,
} from "lucide-react";
...
export default function RetailersPage() {
  ...
  const openEdit = (item) => {
    setEditing(item);
    setForm({ retailerName: item.retailerName ?? "" });
    setFormErr("");
    setFormOpen(true);
  };
  ...
  const save = async () => {
    ...
    try {
      ...
      const body = { RetailerName: name };
      if (editing) {
        await api.put(`/api/retailers/${editing.id}`, body);
      } else {
        await api.post("/api/retailers", body);
      }
  ...
  const remove = async (item) => {
    ...
    try {
      await api.delete(`/api/retailers/${item.id}`);
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
              <div>
                <h3 className="text-[15px] font-semibold text-slate-900">
                  {editing ? "Edit Retailer" : "Add New Retailer"}
                </h3>
                <p className="text-[12px] text-slate-500 mt-0.5">
                  {editing
                    ? `Editing: ${editing.retailerName} · ID: ${editing.id}`
                    : "Fill in the details to create a new retailer."}
                </p>
              </div>
              <button
                onClick={closeForm}
            ...
            <div className="px-6 py-5">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={setField("retailerName")}
                  autoFocus
                />
        ...
        <div className="rounded-xl border border-slate-200 bg-white shadow-sm">
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h2 className="text-[15px] font-semibold text-slate-900">Retailers</h2>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="relative">
                ...
                <input
                  className="h-9 w-56 rounded-lg border border-slate-200 bg-slate-50 pl-9 pr-3 text-sm placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-400/40 focus:border-indigo-400 transition-all"
                  ...
                  <tr>
                    <td colSpan={4} className="px-6 py-12 text-center text-slate-400 text-sm">
                      {search
                        ...
                        : 'No retailers yet. Click "Add New" to create one.'}
                    </td>
                  </tr>
                ) : (
                  items.map((item) => (
